...
Multi-User Real-time: Sistema de salas privadas para tocar com amigos em tempo real usando WebRTC (PeerJS) e BroadcastChannel.
...
Sem Coleta de Dados: Não armazenamos nomes, e-mails, endereços IP ou qualquer informação pessoal identificável.

Ambiente Seguro: Sem chat de texto ou troca de arquivos, eliminando riscos de assédio ou exposição a conteúdo impróprio.

Conexão Direta (P2P): A comunicação ocorre via Peer-to-Peer, sem passar por servidores centrais de armazenamento de dados.

Identidade Animal: Uso de Emojis aleatórios para manter o anonimato de forma lúdica e segura.

📂 Como Hospedar (GitHub Pages)

O projeto é otimizado para ser leve e funcional:

Faça o upload dos arquivos index.html, manifest.json e Service_Worker.js.
...
Rede: PeerJS (P2P) e BroadcastChannel.
